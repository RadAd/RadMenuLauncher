...
public:
    ...
    int ItemFromPoint(POINT p) const { return (int) SendMessage(m_hWnd, LB_ITEMFROMPOINT, 0, MAKELPARAM(p.x, p.y)); }
...
public:
    ...
    {
        int cx = 0, cy = 0;
        CHECK(ImageList_GetIconSize(m_hImageList, &cx, &cy));
        return { cx, cy };
    }
        ...
        if (lpDrawItem->hwndItem == *this)
        {
            ...
            if (m_hImageList != NULL and pData and pData->iIcon >= 0)
            {
                ImageList_DrawEx(m_hImageList, pData->iIcon, lpDrawItem->hDC,
                    ...
                    CLR_DEFAULT, CLR_DEFAULT, ILD_NORMAL | ILD_TRANSPARENT);
            }
